...
结果名称、角色和稀有度由仓库内的版本化卡牌表解析，原始 ID 永远保留。保底按协议的卡池命名空间自动分类：`100xx` 起始契约、`20001` 常规契约、`20002–29999` 常驻遴选契约、`30000–39999` 限时定向契约；其中遴选和限时定向各自在同类更替池之间继承，四种类型互不串线。新一期池无需更新名单即可自动归类；超出已知命名空间的池会保持独立，避免静默误算。
