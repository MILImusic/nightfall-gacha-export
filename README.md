...
2. 打开本工具。游戏可以保持开着，不需要退出或重新登录。
3. 点击“启动连接接管”，在 UAC 窗口中允许管理员权限。
4. 游戏还没开就启动游戏并登录；游戏已经登录着就进入「契约 → 抽卡记录」界面。两种情况接管成功后按钮都会变成“获取全部记录”：
...
原神、崩铁等导出器通常从抽卡页面取得临时 `authKey`，再调用独立的历史 Web API；导出查询和游戏主连接互不抢占。《夜幕之下》的历史命令则复用游戏的 TCP 会话，每页只返回 5 条，代理必须在这条连接建立时就在场，无法从一条已经建立的连接中途安全插入。

好在游戏并不是整局只连一次：**登录时**会建立这条连接，**进入「契约 → 抽卡记录」界面时**也会重新建立一次。因此**游戏不需要退出或重新登录**，只要接管早于其中任意一次即可：

- 游戏还没启动：先点“启动连接接管”，再启动游戏并登录，登录完成时即接管成功。
- 游戏已经登录着：先点“启动连接接管”，再进入「契约 → 抽卡记录」界面，进去时即接管成功。

若进入「契约 → 抽卡记录」时接管还没开着，退回别的页面、再走一次「契约 → 抽卡记录」即可，同样不必重启游戏。
...
- 工具无法从一条已经建立的游戏连接中途插入，接管必须先于连接建立。实际操作上不构成负担：进入「契约 → 抽卡记录」界面会重建连接，先开接管再进该界面即可，游戏无需退出或重新登录。
...
- 游戏主连接：TCP `12090`（默认值；实测部分客户端走 `12085` 等其他端口，v0.1.8 起接管会按游戏进程实际连接的端口自动适配）
- 历史命令：`0x000c08`
