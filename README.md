...
结果名称、角色和稀有度由仓库内的版本化卡牌表解析，原始 ID 永远保留。保底按四种池分别计算：`10001` 起始契约、`20001` 常规契约、`20002–20007` 常驻遴选契约、`30001–30005` 限时定向契约；其中遴选和限时定向各自在同类更替池之间继承，四种类型互不串线。未来出现的新池在规则得到确认前会保持独立，避免误算。
